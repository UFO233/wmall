...
(function (factory) {
	...
	var Tree = function (element, options) {
        ...
        if (this.options['selectable']) {
            ...
            if (this.options.folderSelect) {
                ...
                //this.$element.on('click.fu.tree', '.tree-branch-header', $.proxy( function(ev) { this.openFolder($(ev.currentTarget).parent()); }, this));
                this.$element.on('click.fu.tree', '.tree-branch-header .icon-folder', $.proxy(function (ev) {
                    this.openFolder($(ev.currentTarget).parent());
                }, this));
                this.$element.on('click.fu.tree', '.tree-branch-header .icon-item', $.proxy(function (ev) {
                    this.selectFolder($(ev.currentTarget));
                }, this));
                this.$element.on('click.fu.tree', '.tree-branch-header .tree-label', $.proxy(function (ev) {
                    this.openFolder($(ev.currentTarget).parent());
                }, this));
            }
        } else {
            this.$element.on('click.fu.tree', '.tree-item', $.proxy(function (ev) {
                this.clickItem($(ev.currentTarget));
            }, this));
        }
        this.render();
    };
	...
	Tree.prototype = {
		...
		populate: function ($el) {
            ...
            var parentSelect = this.options['parentSelect'];
            var childSelect = this.options['childSelect'];
                ...
                $.each(items.data, function (index, value) {
                    ...
                    if (value.type === 'folder') {
                        ...
                        if ('additionalParameters' in value
                            && 'children' in value.additionalParameters
                            && 'expand' in value.additionalParameters
                            && value.additionalParameters['children'] == true
                            && value.additionalParameters['expand'] == true) {
                            setTimeout(function () {
                                header.find('.icon-folder').trigger('click')
                            }, 0);
                        }
                        if ('additionalParameters' in value
                            ...
                            setTimeout(function () {
                                header.find('.icon-item').trigger('click');
                            }, 0);
                        }

                        //当父节点为选中时，判断是否需要选中子节点
                        if (parentSelect == true && childSelect == true) {
        ...
        clickItem: function (el) {
            ...
            this.$element.trigger(eventType + '.fu.tree', {target: selData});
        },
        ...
        selectFolder: function (clickedElement) {
            ...
            if ($clickedBranch.hasClass('tree-selected')) {
                ...
                if ($clickedElement.hasClass(this.options['selected-icon']) || $clickedElement.hasClass(this.options['unselected-icon'])) {
                    $clickedElement.removeClass(this.options['selected-icon']).addClass(this.options['unselected-icon']);//ACE
                }
            } else {
                $clickedBranch.addClass('tree-selected');
                if ($clickedElement.hasClass(this.options['selected-icon']) || $clickedElement.hasClass(this.options['unselected-icon'])) {
                    $clickedElement.removeClass(this.options['unselected-icon']).addClass(this.options['selected-icon']);//ACE
                }
            }

            if (!$clickedBranch.hasClass('tree-open')) {
                this.openFolder($clickedBranch);
            }
            if (this.options.multiSelect) {
                if (this.options['parentSelect']) {//父子全选功能
                    var $object = this;
                    var $childFolders = $clickedBranch.children().children("li .tree-branch");
                    $.each($childFolders, function (index, value) {
                        var $value = $(value);
                        if ($value.hasClass('tree-selected') && eventType == 'deselected') {
                            $object.selectFolder($value.find('.tree-branch-header').find('.icon-item'));
                        } else if (!$value.hasClass('tree-selected') && eventType == 'selected') {
                            $object.selectFolder($value.find('.tree-branch-header').find('.icon-item'));
                        }
                    });
                    var $childItems = $clickedBranch.children().children("li .tree-item");
                    $.each($childItems, function (index, value) {
                        var $value = $(value);
                        if ($value.hasClass('tree-selected') && eventType == 'deselected') {
                            $object.selectItem($value);
                        } else if (!$value.hasClass('tree-selected') && eventType == 'selected') {
                            $object.selectItem($value);
                        }
                    });
