...
//时分秒的有效检测
    Dates.timeVoid = function (times, index) {
        ...
        if (Dates.ymd[1] + 1 == Dates.mins[1] && Dates.ymd[2] == Dates.mins[2]) {
        	...
        	if(index === 1 && hms[0].value< Dates.maxs[3]){
        		return 0;
        	}
        	
        	if(index === 2 && (hms[0].value< Dates.maxs[3]||hms[1].value< Dates.maxs[4])){
        		return 0;
        	}
...
//渲染控件骨架
    Dates.view = function (elem, options) {
        ...
        // 设置开始结束时间
        try {
            if (options.max && options.max.indexOf("#")!=-1 ) {
                options.max = $(options.max).val();
            }

            if (options.min && options.min.indexOf("#")!=-1 ) {
                options.min = $(options.min).val();
            }
        } catch (e) {
        }
